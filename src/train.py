...
def plot_history(h1, h2, qwk_history, per_class_history=None, save_dir=OUTPUT_DIR):
    ...
    if per_class_history:
        ...
        # We can infer from keys like 'precision_0', 'precision_1'...
        classes = sorted(list(set(int(k.split('_')[1]) for k in per_class_history.keys() if 'precision' in k)))
        
        # Helper to plot per-class metric
        def plot_per_class(metric_name, title, filename):
            plt.figure(figsize=(10, 6))
            for c in classes:
                key = f'{metric_name}_{c}'
                if key in per_class_history:
                    # History might be inconsistent length if training was interrupted or weird merging happened
                    # But here we pass the cumulative history object from the callback directly
...
def save_class_samples(df, save_dir=OUTPUT_DIR, samples_per_class=5):
    ...
    for cls in unique_classes:
        ...
        for i, (_, row) in enumerate(samples.iterrows()):
            ...
            # Load image using OpenCV (via simple file read for vis) or simple matplotlib
            # We used 'data/train_images/{x}.png' in main
...
def main():
    ...
    # y_true from test_ds (via create_dataset) is now one-hot encoded
    ...
    # helper for standard prediction
    ...
    # 10.1 Train Set (Need unshuffled for ID matching)
    print(" - Predicting on Train set...")
    # Re-create dataset without shuffling
    ...
    # val_ds is already non-shuffled
    ...
    # 10.3 Test Set (Use TTA results)
    print(" - Saving Test set predictions...")
    # y_true and y_pred_probs were calculated in Step 7 with TTA
    # y_true in Step 7 was converted to argmax, but save_predictions expects labels. 
    # Wait, Step 7 did: y_true = np.argmax(y_true, axis=1). 
    # And we have y_pred_probs from TTA.
